...
1. **BIND sub-form priority — resolved by the Phase 2 tail census.** The grammar retains both
   manual-documented forms. The measured concentration was `BIND RUN-UNIT DBNAME <id>` in 96 of 119
   initial `idms_unparsed_tail` records (81%); that form was modelled and the total tail count fell
   from 119 to 9. This is aggregate evidence only; no proprietary source is reproduced. See
   `02-06-SUMMARY.md`, "D-09 grammar extension".

2. **COMMIT option syntax — resolved during research.** The COBOL-specific manual confirms
   `COMMIT [TASK] [ALL]`, both optional and independent. No follow-up remains.

3. **IDMS-04 TestErrorCascade ownership — resolved in the coordinated gortex work.** Phase 2 added
   IDMS DML injection cases and hard parity assertions in the gortex repository, proved RED against
   the pre-IDMS shim and GREEN/deterministic against the refreshed shim. See `02-05-SUMMARY.md`,
   "RED / GREEN evidence" and "Determinism".

## Environment Availability

| Dependency | Required By | Available | Version | Fallback |
|------------|------------|-----------|---------|----------|
| `tree-sitter-cli` (lockfile-pinned; `node_modules/tree-sitter-cli/tree-sitter`) | Regenerating the parser, running corpus/query tests | Available only when this target worktree has local `node_modules`; if absent, setup is required via lockfile-pinned `npm ci` before execution, followed by an exact 0.24.5 version check | 0.24.5 | No global CLI fallback; never use `npm install` |
...
**Target-worktree setup note:** local package installation is not a new dependency. The phase adds
no package; `npm ci` restores the existing lockfile-pinned toolchain only when the target worktree's
CLI binary is absent. Corpus roots and gortex remain explicit execution preconditions.
...
|--------|----------|-----------|-------------------|-------------|
| IDMS-01 | 14 verbs parse to named nodes, no ERROR | corpus | `tree-sitter test -i 'idms'` | ✓ existing; gap closure adds update negatives and navigation selector coverage |
| IDMS-02 | Exact verb/record/set/area/scope contract | scripted exact query | `./run_idms_query_capture.sh` | gap-closure artifact in 02-07; owns temporary fixture |
| IDMS-03 | ACCEPT disambiguation, zero reclassification | corpus + differential | `tree-sitter test -i 'accept'` + D-08 differ/self-test | ✓ existing; 02-07 hardens trust boundary/TSV/regex |
| IDMS-04 | Cascade parity at an IDMS DML injection | Go test (external repo) | isolated-GOWORK `go test ./internal/parser/forest/cobolprobe/ -run TestErrorCascade -count=2 -args -enhanced-parser` | ✓ implemented in 02-05; rerun by 02-11 |
...
| IDMS-07 | Separable commit, no site-specific naming | process + guard | `02-11-closeout.sh` runs literal-range commit-separability and estate guards; human judgment remains only for genericness | gap-closure harness planned |
...
**Historical status:** all five original Wave 0 artifacts were delivered by plans 02-01 through
02-05. Gap-closure plans 02-07 through 02-11 harden and extend them; current ownership and commands
are canonical in `02-VALIDATION.md` and supersede the unchecked historical seed list above.
